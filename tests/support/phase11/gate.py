...
def check_phase_gate(
    *,
    ...
    """Evaluate the Phase 11 logical implementation examples exit gate."""
    _ = os.environ
    checks: list[CheckResult] = []
    ...
    checks.append(
        _path_check(
            "phase11-logical-examples-package",
            "examples_catalog",
            "logical_examples",
    ...
    try:
        ...
        checks.append(
            CheckResult(
                "phase11-examples-catalog-valid",
                "examples_catalog",
                "examples-catalog",
    ...
    for index, (filename, marker) in enumerate(DOC_TOPIC_MARKERS, start=1):
        ...
        checks.append(
            CheckResult(
                f"phase11-topic-{index}-{filename}",
                "documentation",
                filename,
                "passed" if ok else "failed",

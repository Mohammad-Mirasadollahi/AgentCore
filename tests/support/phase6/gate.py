...
from .catalog import PHASE6_DOCS, PHASE_SLICES, ROOT, get_slice
...
def check_phase_gate(
    *,
    ...
    """Evaluate the Phase 6 exit gate for Phases 1 through 5."""
    if run_suites is None:
        run_suites = os.environ.get("AGENTCORE_PHASE6_RUN_SUITES", "").strip() in {"1", "true", "yes"}
    checks: list[CheckResult] = []

    for index, path in enumerate(PHASE6_DOCS, start=1):
        checks.append(_path_check(f"phase6-doc-{index}", "documentation", "phase6", path, "docs/06-technical-logic"))

    for item in PHASE_SLICES:
        checks.append(_path_check(f"src-{item.phase}", "canonical_path", item.service, item.src_path, str(item.logic_doc.relative_to(ROOT))))
        checks.append(_path_check(f"tests-{item.phase}", "canonical_path", item.service, item.test_path, str(item.logic_doc.relative_to(ROOT))))
        checks.append(_path_check(f"logic-{item.phase}", "documentation", item.service, item.logic_doc, str(item.logic_doc.relative_to(ROOT))))
...
def _readme_mentions_command(item) -> CheckResult:
    ...
    if not readme.exists():
        return CheckResult(f"readme-{item.phase}", "canonical_command", item.service, "failed", "service README missing", [str(readme)], doc_ref)
    text = readme.read_text(encoding="utf-8")
    needle = f"tests/backend/{item.service}"
    if needle in text:
        return CheckResult(f"readme-{item.phase}", "canonical_command", item.service, "passed", f"README references {needle}", [str(readme.relative_to(ROOT))], doc_ref)
    return CheckResult(f"readme-{item.phase}", "canonical_command", item.service, "failed", f"README missing canonical test path {needle}", [str(readme.relative_to(ROOT))], doc_ref)

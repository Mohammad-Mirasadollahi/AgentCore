...
def test_external_ticketing_uses_main_service_and_postgres():
    assert os.environ.get("AGENTCORE_ADAPTER_SERVICE_DATABASE_URL"), (
        "AGENTCORE_ADAPTER_SERVICE_DATABASE_URL must point to the main AgentCore PostgreSQL database"
    )
    run_id = uuid4().hex

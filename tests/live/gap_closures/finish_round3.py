...
def ensure_mcp() -> None:
    ...
    run([AGENTCORE, "service", "start", "--json"], env=env, timeout=240)
    for _ in range(60):
        try:
            if httpx.get("http://127.0.0.1:32500/health", timeout=2.0).status_code == 200:
                return
...
def main() -> int:
    log = (ROOT / ".agentcore/run/mcp-http.log").read_text(errors="ignore")
    ok_fail = (
    ...
    token = mint_connect_token(
        tenant_id="mir", workspace_id="dev", project_id="agentcore", ttl_seconds=600
    )

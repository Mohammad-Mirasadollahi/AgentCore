...
os.environ.setdefault("AGENTCORE_NEO4J_PASSWORD", "agentcore-local-dev-secret")
os.environ.setdefault("AGENTCORE_POSTGRES_PASSWORD", "agentcore-local-dev-secret")
os.environ.setdefault("AGENTCORE_NEO4J_BOLT_PORT", "32287")
os.environ.setdefault("AGENTCORE_POSTGRES_PORT", "32232")
...
def round_java_di() -> None:
    ...
    svc = build_service()
    users = (ROOT / ".agentcore/gap-live-fixtures/src/main/java/com/acme/UsersService.java").read_text()
    orders = (ROOT / ".agentcore/gap-live-fixtures/src/main/java/com/acme/OrdersService.java").read_text()
    wire = (ROOT / ".agentcore/gap-live-fixtures/wire/wire_app.go").read_text()
    ...
    if hasattr(svc.store, "list_edges"):
        ...
        uri = f"bolt://127.0.0.1:{os.environ['AGENTCORE_NEO4J_BOLT_PORT']}"
        auth = ("neo4j", os.environ["AGENTCORE_NEO4J_PASSWORD"])
...
def round_mcp_write_gate() -> None:
    ...
    secret = Path("/opt/AgentCore/.agentcore/mcp-http.secret").read_text().strip()
    os.environ["AGENTCORE_MCP_TOKEN_SECRET"] = secret
    token = mint_connect_token(tenant_id="mir", workspace_id="dev", project_id="agentcore", ttl_seconds=3600)
    ...
    gen = rpc(
        "tools/call",
        {
            ...
            "arguments": {
                "server_name": "AgentCore-Programming",
                "tool_name": "agentcore_guidance_resolve",

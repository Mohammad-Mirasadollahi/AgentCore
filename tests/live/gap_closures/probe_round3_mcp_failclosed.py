...
def mint() -> str:
    ...
    return mint_connect_token(
        tenant_id="mir", workspace_id="dev", project_id="agentcore", ttl_seconds=3600
    )
...
def restart_mcp(*, guidance_required: bool) -> None:
    ...
    if guidance_required:
        env["AGENTCORE_GUIDANCE_RESOLVE_REQUIRED"] = "1"
    else:
        env.pop("AGENTCORE_GUIDANCE_RESOLVE_REQUIRED", None)
        env["AGENTCORE_GUIDANCE_RESOLVE_REQUIRED"] = "0"
    # Ensure packages on path for child
    env.setdefault(
        "PYTHONPATH",
        f"{ROOT}/backend/packages:{ROOT}/backend/services/mcp-gateway-service/src",
    )
    cmd = [str(ROOT / ".venv/bin/agentcore"), "service", "restart", "--json"]
...
def main() -> int:
    ...
    env["AGENTCORE_LITELLM_ENABLED"] = "false"
    ingest = subprocess.run(
        [
            str(ROOT / ".venv/bin/agentcore"),
            "graph",
            "ingest",
            "--tenant",
            "mir",
            ...
            "gap-live-cli",
            "--path",
            str(ROOT / ".agentcore/gap-live-fixtures"),
            "--max-files",
            "5",
        ],
        cwd=str(ROOT),

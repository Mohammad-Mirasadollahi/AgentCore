...
Requires ``agentcore service`` up. Uses dedicated project scope.
...
DOC_ORPHAN = "ac.doc.test.stale-live-orphan"
DOC_GHOST = "ac.doc.test.stale-live-ghost"
DOC_WIKI = "ac.doc.test.stale-live-wiki-orphan"
DOC_DUP_A = "ac.doc.test.stale-live-dup-a"
DOC_DUP_B = "ac.doc.test.stale-live-dup-b"
DOC_HEALTHY = "ac.doc.test.stale-live-healthy"
DOC_RELATED_A = "ac.doc.test.stale-live-rel-a"
DOC_RELATED_B = "ac.doc.test.stale-live-rel-b"
...
def test_live_stale_docs_candidates_via_mcp_http():
    ...
    assert "agentcore_docs_stale_candidates" in by_name, search_payload
    assert "agentcore_docs_write" in by_name, search_payload
    server_name = str(by_name["agentcore_docs_stale_candidates"].get("server_name") or "")
    assert server_name
    ...
    def index_doc(
        *,
        ...
        result = execute("agentcore_docs_write", args)
        _index_ok(result)
        if symbol:
    ...
    payload = execute(
        "agentcore_docs_stale_candidates",
        {

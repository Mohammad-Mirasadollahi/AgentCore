...
Requires:
- AgentCore Postgres on 127.0.0.1:32232 (compose)
...
DATA = Path(os.environ.get("AGENTCORE_LIVE_TLS_DATA", "/tmp/agentcore-live-https-qa"))
# Use hostname "localhost" so the auto-generated leaf SAN matches (not bare 127.0.0.1).
HOST = "localhost"
BIND = "127.0.0.1"
PORT = int(os.environ.get("AGENTCORE_LIVE_PROFILE_PORT", "32194"))
DSN = os.environ.get(
    "AGENTCORE_PROJECT_PROFILE_DATABASE_URL",
    "postgresql://agentcore:agentcore-local-dev-secret@127.0.0.1:32232/agentcore",
)
...
def https_profile_server():
    if not _postgres_reachable():
        pytest.skip("AgentCore Postgres not reachable for live HTTPS probe")
    ...
    env.update(
        {
            "AGENTCORE_PROJECT_PROFILE_DATABASE_URL": DSN,
            "AGENTCORE_PROJECT_PROFILE_PORT": str(PORT),
            "AGENTCORE_MCP_TOKEN_SECRET": TOKEN_SECRET,
            "AGENTCORE_CONNECT_BOOTSTRAP_SECRET": BOOTSTRAP_SECRET,
            "AGENTCORE_DATA_ROOT": str(DATA),
...
def test_live_https_auto_cert_bootstrap_and_bearer_status(https_profile_server):
    ...
    with httpx.Client(verify=ctx, timeout=30.0) as client:
        ...
        assert access.startswith("ac1."), body
        ...
        sys.path.insert(0, str(REPO / "backend" / "packages"))
        from agentcore_auth.token_registry import hash_access_token

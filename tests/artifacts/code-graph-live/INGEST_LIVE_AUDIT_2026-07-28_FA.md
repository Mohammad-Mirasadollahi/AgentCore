...
محیط: `/opt/AgentCore`، نسخه محصول `0.1.2`، contract `1`  
...
1. تغییر dimension در `PostgresEmbeddingIndex.ensure_schema()` جدول embedding مشترک را به‌صورت سراسری Drop می‌کند. یکی از تست‌های Live رسمی با `dims=16` همین مسیر را اجرا کرد و embedding تمام Scopeها، از جمله `mir/dev/agentcore`، حذف شد.
...
- `agentcore service status` در visibility واقعی همزمان `Process stopped`، `Reachable yes` و exit code 1 برگرداند. این تناقض lifecycle روی workflow معمول `agentcore sync` اثر گذاشت و sync اولیه را متوقف کرد.
...
روی Scope اصلی `mir/dev/agentcore` بعد از suite:
...
- MCP `2024-11-05`.
- server: `AgentCore-Programming 1.3.1`.
...
- `.agentcore/mcp-http.secret`: mode 0600.
- `.agentcore/tmp_mcp_bearer.txt`: mode 0644 و شامل token منقضی.

...
def test_migrate_skips_when_dest_not_empty(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("agentcore_cli.data_root_migrate.subprocess.run", fake_run)
...
def test_migrate_copies_when_empty_and_volume_exists(tmp_path: Path, monkeypatch):
    ...
    assert any("agentcore_agentcore-postgres-data" in " ".join(c) and c[0:2] == ["docker", "run"] for c in runs)

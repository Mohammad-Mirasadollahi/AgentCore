...
def test_retention_days_env(monkeypatch):
    monkeypatch.delenv("AGENTCORE_FOLLOWUP_TASK_RETENTION_DAYS", raising=False)
    assert retention_days({}) == 30
    assert retention_days({"AGENTCORE_FOLLOWUP_TASK_RETENTION_DAYS": "0"}) == 0
    assert retention_days({"AGENTCORE_FOLLOWUP_TASK_RETENTION_DAYS": "7"}) == 7
    assert retention_days({"AGENTCORE_FOLLOWUP_TASK_RETENTION_DAYS": "x"}) == 30
...
def test_create_sync_followup_preserves_code_debt_when_audit_fails(tmp_path: Path, monkeypatch):
    ...
    import agentcore_cli.commands.quality_audit.collect as collect_mod

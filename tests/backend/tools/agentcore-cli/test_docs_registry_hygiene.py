...
from agentcore_cli.docs_registry_hygiene import (
...
def test_is_docs_registry_fixture_noise_markers():
    ...
    assert not is_docs_registry_fixture_noise(
        symbol_path="backend.packages.agentcore_cli.docs_link_sync.sync_human_docs",
        file_path="backend/packages/agentcore_cli/docs_link_sync.py",
    )
...
def test_quality_audit_mcp_purges_fixture_noise(monkeypatch):
    ...
    dscope = DocsScope("mir", "dev", "agentcore")
    docs.index_symbol(
        dscope,
        "agent",
        "c",
    ...
    backends.core_scope.return_value = SimpleNamespace(
        tenant_id="mir", workspace_id="dev", project_id="agentcore"
    )

    monkeypatch.setattr(
        "agentcore_cli.commands.quality_audit.collect.build_quality_audit_report",
        lambda *a, **k: {
            "findings": [],
    ...
    monkeypatch.setattr(
        "agentcore_cli.commands.quality_audit.mcp_payload.compact_quality_audit_payload",
        lambda report, **k: {
            "findings": [],
            "findings_total": 0,
    ...
    out = quality_mod.quality_audit(
        backends,
        {"top_n": 5},
        scope={"tenant_id": "mir", "workspace_id": "dev", "project_id": "agentcore"},

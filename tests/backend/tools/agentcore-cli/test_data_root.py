"""Tests for AgentCore sibling data-root resolution."""
...
def test_default_data_root_sibling_of_install():
    assert default_data_root(Path("/opt/AgentCore")) == Path("/opt/AgentCore-data")
    assert default_data_root(Path("/home/x/AgentCore")) == Path("/home/x/AgentCore-data")


def test_resolve_data_root_env_override(tmp_path: Path, monkeypatch):
    override = tmp_path / "custom-data"
    monkeypatch.setenv("AGENTCORE_DATA_ROOT", str(override))
    assert resolve_data_root(install_root=tmp_path / "AgentCore") == override.resolve()


def test_resolve_data_root_default_sibling(tmp_path: Path, monkeypatch):
    monkeypatch.delenv("AGENTCORE_DATA_ROOT", raising=False)
...
def test_ensure_stamps_marker_and_migrates_legacy(tmp_path: Path, monkeypatch):
    monkeypatch.delenv("AGENTCORE_DATA_ROOT", raising=False)
    install = tmp_path / "AgentCore"
    (install / ".agentcore" / "mcp-usage").mkdir(parents=True)
    (install / ".agentcore" / "mcp-usage" / "events.jsonl").write_text("x\n", encoding="utf-8")
    root = ensure_data_root(install_root=install)
    marker = install / ".agentcore" / "data-root"

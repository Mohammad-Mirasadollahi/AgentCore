...
from agentcore_cli.connect_config import ConnectSettings, http_error_message, load_connect_settings
...
def test_http_scheme_rejected_without_override(tmp_path, monkeypatch, field):
    ...
    cfg = _write_cfg(
        tmp_path,
        f"server:\n  {field}: http://agentcore.example.internal:9\n"
        "scope:\n  tenant: t\n  workspace: w\n  project: p\n",
    )
...
def test_https_scheme_allowed(tmp_path, monkeypatch):
    ...
    assert settings.api_url == "https://agentcore.example.internal:9"


def test_http_scheme_allowed_with_explicit_override(tmp_path, monkeypatch):
    monkeypatch.setenv("AGENTCORE_ALLOW_INSECURE_HTTP", "1")
    cfg = _write_cfg(
        tmp_path,
        "server:\n  graph_url: http://agentcore.example.internal:9\n"
        "scope:\n  tenant: t\n  workspace: w\n  project: p\n",
    )
    settings = load_connect_settings(config_path=str(cfg), allow_incomplete=True)
    assert settings.graph_url == "http://agentcore.example.internal:9"
...
def test_http_error_message_on_401_points_at_rebootstrap():
    ...
    assert "agentcore connect" in message
...
def test_api_ingest_401_fails_closed_without_retry(monkeypatch):
    from agentcore_cli.connect_flow import api as api_mod
    ...
    with pytest.raises(SystemExit, match="agentcore connect"):

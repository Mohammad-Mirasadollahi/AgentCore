"""Tests for TLS auto-certificate material under the AgentCore data root."""
...
def test_ensure_tls_material_uses_operator_env_paths(
    ...
    monkeypatch.setenv("AGENTCORE_TLS_CERT", str(operator_cert))
    monkeypatch.setenv("AGENTCORE_TLS_KEY", str(operator_key))

    material = ensure_tls_material(data_root=tmp_path / "data", hostname="agentcore.test")
...
def test_ensure_tls_material_key_permissions(tmp_path: Path) -> None:
    ...
    material = ensure_tls_material(data_root=tmp_path, hostname="agentcore.test")
...
def test_ensure_tls_material_renews_near_expiry(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    from agentcore_cli.tls_certs import LEAF_RENEW_BEFORE_DAYS, ensure_tls_material

    first = ensure_tls_material(data_root=tmp_path, hostname="agentcore.test")
    assert first.generated is True

    near_expiry = datetime.now(timezone.utc) + timedelta(days=LEAF_RENEW_BEFORE_DAYS - 1)
    monkeypatch.setattr("agentcore_cli.tls_certs._read_cert_not_after", lambda _p: near_expiry)

    renewed = ensure_tls_material(data_root=tmp_path, hostname="agentcore.test")

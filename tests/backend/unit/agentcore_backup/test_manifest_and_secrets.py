...
from agentcore_backup.bundle import pack_directory, unpack_archive
from agentcore_backup.manifest import (
...
from agentcore_backup.remap import remap_row, resolve_target_scope
from agentcore_backup.scope import Remap, Scope
from agentcore_backup.secrets import assert_no_secrets, find_secret_hit
...
def test_pack_unpack(tmp_path: Path):
    ...
    pack_directory(src, acbak)
    out = unpack_archive(acbak, tmp_path / "out")
...
def test_remap_plain_text_primary_keys_for_same_server_clone():
    ...
    out = remap_row(row, source=source, target=target)
    assert out["id"] == "acbak:t2/w2/p2:mem_abc"
    assert out["memory_id"] == "acbak:t2/w2/p2:mem_abc"
    assert out["title"] == "keep-me"
    assert out["nested"]["id"] == "acbak:t2/w2/p2:mem_abc"
...
def test_restore_replace_requires_yes():
    from agentcore_backup.orchestrator import restore_bundle

    with pytest.raises(ValueError, match="replace requires --yes"):
        restore_bundle(
            Path("/nonexistent.acbak"),
            repo_root=Path("/tmp"),
            replace=True,
            yes=False,
        )

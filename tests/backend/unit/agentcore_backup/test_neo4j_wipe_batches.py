...
from agentcore_backup.neo4j_store import wipe_neo4j
...
def test_wipe_neo4j_batches_until_empty(monkeypatch):
    monkeypatch.setenv("AGENTCORE_NEO4J_PASSWORD", "not-a-placeholder-secret")
    monkeypatch.setenv("AGENTCORE_NEO4J_URI", "bolt://127.0.0.1:7687")
    session = _FakeSession([500, 500, 47, 0])
    monkeypatch.setattr(
        "agentcore_backup.neo4j_store._driver", lambda: _FakeDriver(session)
    )

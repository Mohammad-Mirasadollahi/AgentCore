...
def test_tenancy_mode_default_and_fail_fast(monkeypatch):
    monkeypatch.delenv("AGENTCORE_TENANCY_MODE", raising=False)
    assert resolve_tenancy_mode({}) == "shared_scoped"
    with pytest.raises(ArchitectureGovernanceError):
        resolve_tenancy_mode({"AGENTCORE_TENANCY_MODE": "db_per_tenant"})
    with pytest.raises(ArchitectureGovernanceError):
        resolve_tenancy_mode({"AGENTCORE_TENANCY_MODE": "graph_per_tenant"})
    with pytest.raises(ArchitectureGovernanceError):
        resolve_tenancy_mode({"AGENTCORE_TENANCY_MODE": "deploy_per_customer"})
...
def test_admin_matrix_and_boundary_surfaces():
    ...
    assert guidance_resolve_required({"AGENTCORE_GUIDANCE_RESOLVE_REQUIRED": "1"}) is True

...
```bash
PYTHONPATH=hackathon/backend/change-society-service/src:hackathon/sdk/python .venv/bin/python -m pytest tests/backend/legacy/change-society-service -q
```

Optional live Qwen gate (skipped without `QWEN_API_KEY`):

```bash
QWEN_API_KEY=... PYTHONPATH=hackathon/backend/change-society-service/src:hackathon/sdk/python .venv/bin/python -m pytest tests/backend/legacy/change-society-service/test_qwen_live.py -q
```

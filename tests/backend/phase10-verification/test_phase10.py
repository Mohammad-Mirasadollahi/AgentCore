...
from phase10.catalog import REQUIRED_DOCS
from phase10.checks import run_all_checks
from phase10.gate import check_phase_gate, explain_failed_check
...
def test_phase10_gate_passes_docs_and_gap_register():
...
def test_phase10_verification_checks_exit_criteria():
...
def test_waiver_marks_gate_waived_when_forced_failure(monkeypatch):
    ...
    decision = check_phase_gate(waiver_ref="issue:phase10-temp")
    assert decision.status == "waived"
    explained = explain_failed_check(decision, "phase10-doc-1")

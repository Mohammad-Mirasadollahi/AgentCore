...
def test_agentcore_flag_change_different_hash(lang: str, a: str, b: str) -> None:
    assert content_hash(_read(a), lang)["hash"] != content_hash(_read(b), lang)["hash"]
...
def test_python_flags_ignore_string_literals() -> None:
    src = 'msg = "agentcore: fake"\n# agentcore: real=1\n'
    flags = extract_agentcore_flags(src, "python")
    assert flags == ["# agentcore:real=1"]

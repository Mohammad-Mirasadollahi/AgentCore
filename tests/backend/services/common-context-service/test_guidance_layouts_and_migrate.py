...
def test_layout_profile_claude_rule_and_skill_paths():
    ...
    rule = {"item_type": "always_rule", "slug": "mcp-first-agentcore", "body": "# r\n"}
    skill = {"item_type": "skill", "name": "agentcore-session-bootstrap", "body": "# s\n"}
    assert relative_paths_for_item(rule, "claude_compatible") == [
        ".claude/rules/mcp-first-agentcore.md"
    ]
    assert relative_paths_for_item(skill, "claude_compatible") == [
        ".claude/skills/agentcore-session-bootstrap/SKILL.md"
    ]
...
def test_materialize_claude_compatible_writes_claude_md(tmp_path):
    ...
    assert (root / ".claude/rules/mcp-first-agentcore.md").is_file()
    assert (root / ".claude/skills/agentcore-session-bootstrap/SKILL.md").is_file()
    # Cursor mdc frontmatter must not appear on Claude layout rules
    body = (root / ".claude/rules/mcp-first-agentcore.md").read_text(encoding="utf-8")

...
def test_superseded_retrieval_risk_historical():
    doc = _doc(
        ...
        "docs/example/old.md",
        linked=["pkg.mod.live"],
        fm={"lifecycle_lane": "historical", "superseded_by": "ac.doc.test.newer"},
    )
...
def test_duplicate_authority_skipped_when_related_docs():
    a = _doc(
        ...
        fm={
            "authority": "normative",
            "lifecycle_lane": "current",
            "doc_id": "ac.doc.test.rel-a",
            "related_docs": ["ac.doc.test.rel-b"],
            "updated_at": "2020-01-01",
        },
    )
    b = _doc(
        ...
        fm={
            "authority": "normative",
            "lifecycle_lane": "current",
            "doc_id": "ac.doc.test.rel-b",
            "related_docs": ["ac.doc.test.rel-a"],
            "updated_at": "2020-01-01",
        },
    )
    sym = _sym("s1", symbol_path="pkg.mod.Shared")
...
def test_healthy_anchored_doc_not_flagged():
    ...
    rows = list(out["candidates"]) + list(out["skipped_uncertain"])
    assert not any(r.get("doc_id") == "ac.doc.test.healthy" for r in rows)

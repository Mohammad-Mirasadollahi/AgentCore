...
def test_build_judge_selects_heuristic_by_default(monkeypatch):
    monkeypatch.delenv("AGENTCORE_RULE_JUDGE", raising=False)
...
def test_build_judge_selects_litellm(monkeypatch):
    monkeypatch.setenv("AGENTCORE_RULE_JUDGE", "litellm")
...
def test_build_judge_rejects_unknown():
    ...
    with pytest.raises(RuntimeError, match="Unsupported AGENTCORE_RULE_JUDGE"):
        build_judge(settings)


def test_settings_from_environment_reads_rule_judge(monkeypatch):
    monkeypatch.setenv("AGENTCORE_RULE_ENGINE_DATABASE_URL", "postgresql://localhost/ac")
    monkeypatch.setenv("AGENTCORE_RULE_JUDGE", "LiteLLM")

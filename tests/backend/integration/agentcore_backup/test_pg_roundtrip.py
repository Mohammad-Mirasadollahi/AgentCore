...
from agentcore_backup.orchestrator import export_bundle, restore_bundle, scope_is_nonempty
from agentcore_backup.pg import connect, database_url
...
def _load_dogfood_database_url() -> None:
    """Adopt local dogfood DB/Neo4j env from .agentcore/mcp-servers.json when present."""
    path = _REPO / ".agentcore" / "mcp-servers.json"
    if not path.is_file():
        return
    try:
...
def test_core_data_export_restore_roundtrip(tmp_path: Path, monkeypatch):
    ...
    acbak = tmp_path / "rt.acbak"
    exported = export_bundle(scope, acbak, repo_root=tmp_path)
...
def test_conflict_fail_closed(tmp_path: Path, monkeypatch):
    ...
    acbak = tmp_path / "cf.acbak"
    export_bundle(scope, acbak, repo_root=tmp_path)

    with pytest.raises(ValueError, match="not empty"):
        restore_bundle(acbak, repo_root=tmp_path, replace=False, yes=False)

    # replace path
    restored = restore_bundle(acbak, repo_root=tmp_path, replace=True, yes=True)
    assert restored["ok"] is True

---
doc_id: ac.doc.rules.llm-judge-operating
...
Close **GAP-T05** by defining how AgentCore runs LLM-as-a-Judge so verdicts are structured, fail-closed on ambiguity, and reproducible from stored metadata.
...
- Bootstrap default remains `HeuristicJudge` (no network). Production LLM path requires `AGENTCORE_RULE_JUDGE=litellm`.

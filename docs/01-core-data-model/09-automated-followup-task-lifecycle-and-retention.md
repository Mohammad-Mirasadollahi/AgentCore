---
doc_id: ac.doc.core.automated-followup-task-lifecycle-and-retention
...
- backend/packages/agentcore_cli/followup_task_lifecycle.py::adopt_legacy_quality_tasks
- backend/packages/agentcore_cli/sync_followup_tasks.py::create_sync_followup_tasks
...
Define how AgentCore **must** manage durable `Task` records created automatically
...
`.agentcore/quality-followup-tasks.json` is a **convenience snapshot only**;
...
| --- | --- | --- |
| `sync-followup` | `agentcore sync` → `create_sync_followup_tasks` | standards-skipped docs/code; code.sync_debt aggregates |
| `mcp-quality` | `agentcore_quality_audit` with `create_tasks=true` | per-path high/medium quality findings |
...
| 4 | Mirror write (sync only) | Specs still owed | `.agentcore/quality-followup-tasks.json` | Soft-fail |
...
Reconcile **must** run on every `agentcore sync` that reaches follow-up handling,
...
- Environment: `AGENTCORE_FOLLOWUP_TASK_RETENTION_DAYS` (integer ≥ 0).
...
| --- | --- | --- | --- |
| `agentcore sync` follow-up | Yes (if specs) | Always (`sync-followup` only) | Always (if retention days > 0) |
| `agentcore_quality_audit` + `create_tasks=true` | Yes (high/medium) | Yes (`mcp-quality` only) | Yes (if retention days > 0) |
| `agentcore_quality_audit` without create | No | Optional (`reconcile_tasks=true`) | Optional same flag |
| `agentcore followup-tasks reconcile` | No | Yes (origin filter) | No |
| `agentcore followup-tasks purge` | No | No | Yes (`--days` / env) |
| `agentcore followup-tasks adopt-legacy` | Stamp metadata | Cancel fingerprint dupes | No |
...
```text
agentcore followup-tasks list [--status open|terminal|all] [--origin sync-followup|mcp-quality|all]
agentcore followup-tasks status [--origin …]
agentcore followup-tasks adopt-legacy [--dry-run|--yes]
agentcore followup-tasks reconcile [--origin …] [--dry-run]
agentcore followup-tasks purge [--days N] [--dry-run|--yes]
```
...
Operator catalog entry: `docs/08-software-engineering-architecture/42-agentcore-cli-command-reference-continued-continued-continued.md`
(`agentcore followup-tasks`). MCP: `agentcore_quality_audit` args `create_tasks` /
...
| Area | Path |
| --- | --- |
| Shared policy | `backend/packages/agentcore_cli/followup_task_lifecycle.py` |
| Sync producer | `backend/packages/agentcore_cli/sync_followup_tasks.py` |
| CLI operator | `backend/packages/agentcore_cli/commands/followup_tasks.py` + `parser/governance.py` |
...
| Tests | `tests/backend/tools/agentcore-cli/test_followup_task_lifecycle.py`, `test_followup_tasks_cli.py` |

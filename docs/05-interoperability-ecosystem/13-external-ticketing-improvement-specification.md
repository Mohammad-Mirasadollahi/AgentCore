---
doc_id: ac.doc.interop.external-ticketing-improvement-specification
...
related_docs:
- ac.doc.interop.external-ticketing-live-quality-assessment
...
- Replacing AgentCore-native Issue, Task, or AgentTicket aggregates.
- Making Jira, Linear, GitHub, or another tracker the AgentCore system of record.
...
| `assignee_ref` | Stable external or AgentCore subject reference |
...
| Vendor | Required env |
|---|---|
| Jira | `AGENTCORE_JIRA_BASE_URL`, `AGENTCORE_JIRA_EMAIL`, `AGENTCORE_JIRA_API_TOKEN`, `AGENTCORE_JIRA_PROJECT_KEY` |
| Linear | `AGENTCORE_LINEAR_API_KEY`, `AGENTCORE_LINEAR_TEAM_ID` (+ `AGENTCORE_LINEAR_DONE_STATE_ID` for status push) |
| GitHub Issues | `AGENTCORE_GITHUB_TOKEN`, `AGENTCORE_GITHUB_OWNER`, `AGENTCORE_GITHUB_REPO` |

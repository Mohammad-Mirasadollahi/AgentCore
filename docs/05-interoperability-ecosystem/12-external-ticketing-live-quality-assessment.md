---
doc_id: ac.doc.interop.external-ticketing-live-quality-assessment
...
related_docs:
- ac.doc.adapter.phase-5-api-contract
- ac.doc.interop.external-ticketing-improvement-specification
...
The short live scenario ran the real FastAPI application from `backend/services/adapter-service/src/adapter_service/api.py::build_app` against the active AgentCore PostgreSQL database.

Test scope:

```text
tenant_id=live-audit
workspace_id=agentcore
...
| Vendor integration proof | Incomplete | The live scenario verified AgentCore persistence, not a real Jira, Linear, or equivalent remote API |
...
| TQ-005 | Resolved at the AgentCore boundary | Independent dispatch state, result callback, failure/dead-letter evidence, retry, and outbox `TicketDispatchHandler`; opt-in vendor sandbox remains external verification |
...
- [AgentCore API Catalog](../14-api-design-and-naming-standards/04-agentcore-api-catalog.md)

---
doc_id: ac.doc.interop.sdk-release-and-adapter-harness
...
summary: Resolve GAP-T06 — Python and TypeScript ship first, private registry names
  agentcore-sdk / @agentcore/sdk, OpenAPI generator ownership, and the adapter capability
...
visibility: internal
linked_symbols:
- backend/packages/agentcore_sdk/client.py::AgentCoreClient
...
- tests/backend/packages/test_agentcore_sdk.py::test_agentcore_sdk_requires_base_url
related_docs:
- ac.doc.interop.sdk-and-developer-platform
- ac.doc.interop.agent-communication-language-runtime-sdk
...
flowchart TD
  ...
  stubs --> py[agentcore-sdk Python]
  stubs --> ts["@agentcore/sdk TypeScript"]
...
| --- | --- | --- |
| Python | `agentcore-sdk` (import `agentcore_sdk`) | `backend/packages/agentcore_sdk/` via `pip install -e '.[sdk]'` |
| TypeScript | `@agentcore/sdk` | `backend/packages/sdk/typescript/` |
...
| --- | --- | --- |
| Python | Private index (org PyPI / Artifactory) | `agentcore-sdk` |
| TypeScript | Private npm scope | `@agentcore/sdk` |

Rules:

- Do not publish AgentCore SDK packages to public npm or public PyPI without explicit per-action consent.
- Version with semver aligned to the control-plane `agentcore` version when possible.
...
| Hand-written transport clients | Platform SDK maintainers | `agentcore_sdk`, `sdk/typescript/src/` |
...
Generated clients do **not** replace domain helpers. Transport clients (`AgentCoreClient`) own URL joining, `X-Correlation-Id`, and `Idempotency-Key` parity across languages.
...
- `pytest tests/backend/packages/test_sdk.py tests/backend/packages/test_agentcore_sdk.py tests/backend/packages/test_sdk_generation.py tests/backend/packages/test_adapter_harness.py`
- Regenerate stubs and confirm the generation test passes.
- Confirm TypeScript package name is `@agentcore/sdk` and Python import is `agentcore_sdk`.
...
- `backend/packages/agentcore_sdk/README.md` — Python package map.

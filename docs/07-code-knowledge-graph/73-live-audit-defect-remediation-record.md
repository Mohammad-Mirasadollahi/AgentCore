---
doc_id: ac.doc.ckg.live-audit-remediation-record
...
- backend/packages/agentcore_cli/util.py::ensure_service_import_paths
related_docs:
...
- `AGENTCORE_EMBEDDING_REFRESH_WORKERS` is configurable and capped at `16`; the observed run used `12`.
...
| Remediation | Patch service-runtime `mcp_status`, execute live approval with a temporary `AGENTCORE_ROOT`, and use an isolated project for the unknown-symbol docs-drift scenario |
...
| Field | Record |
|---|---|
| Symptom | A live approval test failed with `ModuleNotFoundError: code_graph_service` when `AGENTCORE_ROOT` pointed to a temporary directory |
...
| Primary file | `backend/services/agentcore-cli/src/agentcore_cli/util.py` |

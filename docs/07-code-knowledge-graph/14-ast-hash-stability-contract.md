---
doc_id: ac.doc.ckg.ast-hash-stability
...
summary: Normative contract for language-correct content hashing used by Code-Knowledge Graph
  incremental ingest — normalization rules, AgentCore doc-flag comments, generated-file policy,
...
edits do not re-index files, while semantic edits and AgentCore doc-flag comment changes do.
Resolves GAP-T01.
...
flowchart TD
  ...
  astPath --> flags[Append canonical agentcore flags]
  norm --> flags
...
- Ordinary comments that are **not** AgentCore doc flags
...
- AgentCore doc-flag comment text changes (see below)
- Language switch for the same bytes (different normalizer)

### Python

1. Extract `# agentcore:` comments via `tokenize` (string-safe).
...
## AgentCore Doc-Flag Comments

Comments that **must** affect the hash (markers, case-insensitive):

- Python: `# agentcore: ...`
- C-family: `// agentcore: ...` or `/* agentcore: ... */`

Canonical form in the hash input: `# agentcore:<body>`.
...
Generated sources are not exempt from flag rules: an `# agentcore:` marker inside a generated
...
3. AgentCore doc-flag edit → **different** hash

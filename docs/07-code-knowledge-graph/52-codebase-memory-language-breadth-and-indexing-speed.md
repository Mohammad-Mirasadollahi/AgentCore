---
doc_id: ac.doc.ckg.codebase-memory-language-breadth-and-speed
...
summary: Explains how DeusData codebase-memory-mcp reaches ~158 languages and extreme
  ...
  to AgentCore Neo4j CKG without vendoring the C binary.
tags:
- code-intelligence
- codebase-memory
- prior-art
...
- ac.doc.codegraph.language-support-policy
- ac.doc.codegraph.ast-and-lsp-hybrid-parsing-adr
...
- type: constrains
  target: ac.doc.codegraph.language-support-policy
chunk_hints:
  strategy: heading_h2
  max_tokens: 800
  overlap_tokens: 64
...
achieves its published **indexing / query speed**, then map what AgentCore should
**adopt as ideas**, **adapt**, or **avoid**.

This is prior-art analysis only. AgentCore does **not** vendor the C binary,
...
**their** claims (README / BENCHMARK / arXiv:2603.27277), not AgentCore SLOs.
...
| AgentCore policy | Ideas only; clean-room on Neo4j (`44`–`47`); language growth via [`10`](10-language-support-policy.md) |
...
### AgentCore mapping (language breadth)

| Upstream choice | AgentCore stance |
| --- | --- |
...
| Quality tiers / honesty | **Adopt** — publish AgentCore language tiers from our evals, not upstream % |
...
(≈99% reduction). Treat as prior-art motivation, not AgentCore marketing copy
...
### AgentCore mapping (speed)

| Upstream lever | AgentCore stance |
| --- | --- |
...
| Ignore layers (gitignore / project ignore) | **Shipped:** sync merges `.gitignore` + `.agentcoreignore` (CI-45); see [`53`](53-repomix-prior-art-ideas-and-license.md) |
| CPU/worker budgeting | AgentCore path: [`50`](50-sync-cpu-budget-and-store-concurrency-lld.md) |

## Transfer Checklist (normative for engineers)

When citing CBM language breadth or speed in AgentCore designs:

1. Attribute MIT DeusData; link this doc + `21` + `THIRD_PARTY_NOTICES`.
2. Do not promise 158 languages or 3-minute kernel indexes as AgentCore SLOs.
...
- [`10-language-support-policy.md`](10-language-support-policy.md) — AgentCore language matrix
...
- [`50-sync-cpu-budget-and-store-concurrency-lld.md`](50-sync-cpu-budget-and-store-concurrency-lld.md) — AgentCore sync speed levers

---
doc_id: ac.doc.ckg.stale-documentation-candidates-and-cleanup-loop
...
summary: Normative design and implementation for graph- and catalog-backed stale
  ...
  and the closed cleanup loop. Sister to dead-code candidates (doc 36). AgentCore
...
- backend/packages/agentcore_cli/commands/quality_audit/categories.py
...
- ac.doc.docs-sync.feature-specification
...
relations_declared:
- type: complements
  target: ac.doc.ckg.dead-code-cleanup-loop
- type: depends_on
  target: ac.doc.ckg.hybrid-documentation-coverage
- type: depends_on
  target: ac.doc.docs-sync.index
- type: complements
...
This document specifies **stale-documentation intelligence** for the AgentCore wedge: detect documentation candidates that are orphaned, ghost-linked, hash-stale, or unsafe to retrieve as current truth, attach **numeric confidence scores** and **evidence chains**, guide connected coding agents to remediate or retire proven-stale docs in the same change when appropriate, and measure cleanup outcomes.
...
**Implementation status:** Full finding-kind set shipped; precision hardened after live dogfood (skip normative / `future` / `historical` empty-link orphans; resolve path-only and `path::Class.method` `linked_symbols`; ignore blank tokens; path-only unresolved ≠ ghost). Scorer, MCP `agentcore_docs_stale_candidates`, skill/rule/audit hint, unit + live HTTP probe.

AgentCore is not the executor. External IDE assistants and agent runtimes edit or delete Markdown. AgentCore owns candidates, scoring, guidance seed content, freshness/catalog signals, and evidence for benefit measurement.
...
- AgentCore auto-deleting or rewriting Markdown on disk.
- Inventing `DOCUMENTED_BY` edges without Phase 2 resolve / evidence `linked_symbols`.
- Replacing Full-tier documentation authoring (`agentcore-documentation-authoring`) or Body-tier docs-sync validate.
...
| `agentcore_docs_drift_check` / `docs_status` | Drift and coverage status | Optional deep-link into scored stale candidates |
...
```mermaid
flowchart LR
  sync[agentcore sync Phase1+2] --> graph[CKG + Doc registry]
...
| 2 | AgentCore | Stale-doc query + score | Ranked candidates with evidence |
| 3 | Guidance | Skill `agentcore-remove-stale-docs` | Agent proves then remediates |
...
| Guide | Seed always-on rule + skill `agentcore-remove-stale-docs` | Follows rule in the same coding/docs change |
...
Tool name: `agentcore_docs_stale_candidates`  
...
```json
{
  ...
  "candidates": [
    {
      "doc_id": "ac.doc.example.retired-feature",
...
1. Seed skill **`agentcore-remove-stale-docs`**: after code replace/retire **and** after material doc edits, call stale-docs candidates; prefer `safe_to_update` / `safe_to_unlink`; delete only when `safe_to_delete` and score ≥ 0.8; same-change when possible; graph/catalog SoT — not Memory.
...
3. Quality-audit category: `docs.stale_cleanup_hint` — after linking-gap inventory, point agents at `agentcore_docs_stale_candidates`.
...
6. Run `agentcore docs-standards` / quality-audit on touched paths.
...
| 2 | MCP `agentcore_docs_stale_candidates` | [x] | gateway `docs.py` + profile + dispatch |

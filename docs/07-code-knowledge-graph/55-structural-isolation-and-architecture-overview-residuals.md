---
doc_id: ac.doc.ckg.structural-isolation-residuals
...
1. Run `agentcore service restart` (or equivalent) so code-graph + MCP HTTP pick up new code.
2. Trigger repo sync so hash-stable edgeless FILEs enqueue edge repair.
3. Call `agentcore_code_graph_architecture_overview` and inspect `knowledge_gaps`.

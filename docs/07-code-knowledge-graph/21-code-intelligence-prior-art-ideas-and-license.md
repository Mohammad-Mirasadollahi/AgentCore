...
summary: Transferable product and engineering ideas from CodeGraph, code-review-graph,
  graphify, DeusData codebase-memory-mcp, and yamadashy/repomix for AgentCore code
...
- type: complements
  target: ac.doc.ckg.repomix-prior-art-ideas-and-license
chunk_hints:
  strategy: heading_h2
  max_tokens: 800
  overlap_tokens: 64
...
**Default AgentCore policy:** inspire and **re-implement** against AgentCore ports (Neo4j `CodeSymbol`/`CODE_REL`, LiteLLM, MCP gateway, native compression per `54`). Do **not** add these packages as runtime dependencies unless an ADR explicitly accepts MIT/Apache vendoring and SBOM updates.
...
| CI-45 | Layered ignore (hardcoded → `.gitignore` → project ignore) | Adopt | **Shipped:** sync merges `.gitignore` + `.agentcoreignore` |
...
| CI-47 | Per-account coordination daemon + shared watchers | Avoid | AgentCore server sync / explicit `sync`; no host-wide CBM daemon |
...
| ID | Idea | Tag | AgentCore mapping |
| --- | --- | --- | --- |
| RM-02 / RM-03 | Token counts + token-budget exit | Adopt | **Shipped:** explore `estimated_tokens`; `agentcore pack review --token-budget` |
| RM-04 | Token hotspot tree | Adopt | **Shipped:** pack review `hotspots` |
| RM-05 / RM-06 | Layered ignore + secret scan before pack | Adopt | **Shipped:** `.gitignore`/`!`/`.agentcoreignore`; pack review fail-closed scan |
...
| ID | Idea | Tag | AgentCore mapping |
| --- | --- | --- | --- |
| HR-01 / HR-02 | Compress before LiteLLM; content-aware JSON/code/prose | Adopt | **Native in AgentCore** MCP gateway + LLM port |
| HR-03 / HR-04 | CCR retrieve + AgentCore MCP compress/retrieve/stats | Adopt | Product tools; not `ai-toolstack` |
| HR-05 | In-process library | Adopt | Preferred embed shape |
| HR-06 / HR-08 | External LLM proxy wrap; upstream cross-agent memory | Avoid | LiteLLM-only; AgentCore memory BC |
...
| Platform fit | Avoid list + Adapt tags | Neo4j SoR, LiteLLM, tenant scope; native compress in AgentCore |
...
- [ ] Benchmarks do not claim circular graph-derived “100% recall” or paper 83%/10× figures as AgentCore product metrics without our eval.
...
- [ ] Native context compression is an **AgentCore product** obligation ([`54`](54-headroom-native-context-compression.md)); IDE helper stacks are not a substitute.

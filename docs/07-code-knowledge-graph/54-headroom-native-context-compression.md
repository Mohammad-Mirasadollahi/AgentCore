...
- ac.doc.ckg.third-party-notices
...
- type: complements
  target: ac.doc.ckg.third-party-notices
chunk_hints:
  strategy: heading_h2
  max_tokens: 800
  overlap_tokens: 64
...
# 54 - Headroom Native Context Compression For AgentCore

## Purpose

Require that **the AgentCore software product** (platform services, MCP gateway, and
...
State explicitly what is **in scope** (AgentCore) and **out of scope** (separate
...
## Normative product law (AgentCore must use this)

1. **Consumer is AgentCore.** Connected coding agents, MCP clients of AgentCore, and
   AgentCore-internal LLM jobs **must** benefit from native compression when payloads
   are large and low-signal — not from a side IDE plugin as the product SoT.
2. **Native means in-process or AgentCore-owned service.** Compression runs inside
   AgentCore deployment boundaries (for example a library used by `mcp-gateway-service`
...
4. **Local-first / sovereignty.** Originals and compressed forms stay on AgentCore
...
6. **Reversible when lossy.** If compression drops detail, AgentCore **must** expose a
...
If AgentCore vendors or redistributes Headroom source/binaries (or creates Derivative
...
**Default AgentCore policy:** implement a **clean-room** compression lane matching the
idea catalog below, **or** accept `headroom-ai` (or a pinned subset) via ADR + SBOM +
notices. Either path still satisfies “AgentCore must use compression natively.”

## Out of scope (explicit)

| Item | Why excluded |
| --- | --- |
| `ai-toolstack` / Cursor `mcp-lazy` Headroom backend | Separate developer tooling; not AgentCore product runtime |
| `headroom wrap cursor` / agent wrap that routes around LiteLLM | Violates LLM gateway law |
| Upstream cross-agent memory as AgentCore Memory BC | Different bounded context; use AgentCore memory services |
...
proxy, and MCP — **AgentCore adopts the library/service pattern on its own seams**,
not the external wrap/proxy product shape.

```mermaid
flowchart TD
  tools[AgentCore MCP / tool / pack results]
...
| HR-04 | MCP compress / retrieve / stats surface | Adopt | **Shipped:** `agentcore_context_*` on programming profile |
...
| HR-08 | Cross-agent shared memory store | Avoid | Use AgentCore memory BC |
...
| HR-11 | Skip already-compressed / watermarked lanes | Adopt | **Shipped:** skip when `[agentcore_context:` present |

## Ownership (implementation targets)

| Concern | Owner (AgentCore) | Status (2026-07-25) |
| --- | --- | --- |
| Compress/retrieve API + TTL cache | `backend/packages/context_compression/` | Shipped (clean-room) |
| MCP tools | `agentcore_context_compress` / `retrieve` / `stats` on `programming-cursor-mcp` | Shipped |
| CLI measure/stats | `agentcore context measure` / `agentcore context stats` | Shipped |
| Package install | `context_compression` in root `pyproject.toml` packages | Shipped |
| Wire on LLM message assembly | `backend/packages/llm_gateway/gateway.py` (`AGENTCORE_CONTEXT_COMPRESS`) | Shipped |
...
agentcore context measure --file /path/to/blob.json
agentcore context measure --file /path/to/blob.json --json

# Accumulated CLI totals (.agentcore/cache/context-compression-metrics.json)
agentcore context stats

# MCP (after Reload of AgentCore-Programming profile ≥ 1.3.1)
# agentcore_context_compress / agentcore_context_retrieve / agentcore_context_stats
```

Env: `AGENTCORE_CONTEXT_COMPRESS` (default on), `AGENTCORE_CONTEXT_COMPRESS_MIN_CHARS` (default 2000). See `.env.example`.
...
1. A documented AgentCore API (HTTP and/or MCP) can compress and retrieve a fixture JSON blob without calling `ai-toolstack`. **Met** (MCP tools).
...
3. Docs and guidance state AgentCore—not IDE toolstack—as the SoT for product compression. **Met** (this doc).

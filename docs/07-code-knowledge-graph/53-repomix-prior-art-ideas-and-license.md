---
doc_id: ac.doc.ckg.repomix-prior-art-ideas-and-license
...
summary: Transferable ideas from yamadashy/repomix (MIT) for AI-friendly repository packing,
  token budgeting, secret scanning, and compress-by-structure — mapped to AgentCore with
...
AgentCore may adopt for **AI-oriented repository packing**, **token budgeting**,
**secret-safe export**, and **structure-preserving compression**. State MIT / IP rules
and map each idea to AgentCore surfaces (context packs, explore skeletonization,
LiteLLM, sovereignty).

Default policy: **ideas only / clean-room**. Do not vendor the Repomix npm package or
CLI into AgentCore unless an ADR + SBOM + notice update explicitly approve it.
...
| --- | --- |
| Pack shape (XML preamble, directory tree, per-file sections) | Pasting Repomix TypeScript into AgentCore |
| Token-budget CI gate concepts | Shipping `repomix` as a required AgentCore runtime dep |
| Secretlint-before-export UX | Claiming “powered by Repomix” |
| Tree-sitter compress (signatures vs bodies) as a pattern | Hosted website packing of private AgentCore trees |
...
- Secretlint security scan (skippable via flag — AgentCore must not skip by default)
...
| RM-03 | `--token-budget` non-zero exit when over limit | Adopt | **Shipped:** `agentcore pack review --token-budget` |
| RM-04 | Token-count tree (hotspot files ≥ N tokens) | Adopt | **Shipped:** `pack review` `hotspots` + `--hotspot-min-tokens` |
| RM-05 | Layered ignore (defaults → gitignore → project ignore) | Adopt | **Shipped:** `.gitignore` + `.agentcoreignore` (+ `!` reinclude) |
...
| RM-20 | Ship Repomix MCP as AgentCore peer by default | Avoid | Prefer AgentCore code-graph MCP; optional external tool is user choice |
| RM-21 | Watch-mode continuous re-pack daemon | Avoid / Adapt | Avoid as graph SoR; optional export watcher only if product asks |
| RM-22 | Auto-generate Claude skills from packed dump | Adapt | Prefer AgentCore guidance skills grounded in graph/docs, not whole-repo paste |

## Mapping To AgentCore Levers
...
## Shipped AgentCore surfaces (2026-07-25)

Clean-room implementation lives in `backend/packages/repo_pack/` (listed in root `pyproject.toml`).

| Capability | Surface |
| --- | --- |
| Layered ignore (RM-05 / CI-45) | Sync merges `.gitignore` then `.agentcoreignore` into `exclude_globs` |
| Secret scan fail-closed (RM-06) | `agentcore pack review` (default); `--allow-secrets` opt-out |
...
| Class | Status |
| --- | --- |
| **Adopt** rows above | Complete for AgentCore (clean-room) |
...
```bash
agentcore pack review --files path/a.py,path/b.py --token-budget 8000 --json
agentcore pack review --from-git --include-diff --out /tmp/review.md
```

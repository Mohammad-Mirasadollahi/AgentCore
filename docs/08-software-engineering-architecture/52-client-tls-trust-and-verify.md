---
doc_id: ac.doc.sea.client-tls-trust-and-verify
...
summary: >-
  Operator guide for AgentCore client HTTPS trust — default tls_verify off (encrypt
...
linked_symbols:
- backend/packages/agentcore_cli/connect_http.py::httpx_verify
- backend/packages/agentcore_cli/connect_http.py::parse_tls_verify
- backend/packages/agentcore_cli/connect_config.py::ConnectSettings
...
Explain how a coding-agent host (client) trusts an AgentCore server over HTTPS:
...
| **Verify on** | `auth.tls_verify: true` + `auth.ca_file` | Client validates the server cert against the AgentCore **private CA PEM** |
...
```text
Client app repo                         AgentCore server
.agentcore/connect.yaml                 {data-root}/certs/
...
Fresh server install / `agentcore service start` auto-creates those certs under the
durable data root (sibling `<install>-data` by default) when missing. Upgrade
**preserves** existing certs.

## Client config (`connect.yaml`)

Path: `<your-app>/.agentcore/connect.yaml` (gitignored).
...
```bash
# On AgentCore server
sudo cat /opt/AgentCore-data/certs/ca.pem
# or: scp root@agentcore-host:/opt/AgentCore-data/certs/ca.pem ./
```

2. On the **client** app repo:

```bash
mkdir -p .agentcore/certs
# place ca.pem there (mode 0644 is fine — it is a public certificate)
cp /path/from/server/ca.pem .agentcore/certs/ca.pem
```
...
```bash
export AGENTCORE_CONNECT_CA_FILE=/secure/path/ca.pem
export AGENTCORE_CONNECT_TLS_VERIFY=true
```
...
`.agentcore/certs/ca.pem` and can point `auth.ca_file` at it. **Verification stays
...
| MCP Streamable HTTP | 32500 | HTTPS by default (`AGENTCORE_MCP_TLS=0` disables) |

Set advertise host for clients:

```bash
# Server .env (example)
AGENTCORE_PUBLIC_HOSTNAME=192.168.1.150
AGENTCORE_MCP_HTTP_PUBLIC_URL=https://192.168.1.150:32500
```

Then restart MCP: `agentcore service restart`.
...
agentcore-client sync --allow-cloud-llm
...
Plain `http://` URLs are still rejected unless `AGENTCORE_ALLOW_INSECURE_HTTP=1`
...
1. `bash install.sh --role server` (or `get-agentcore.sh`)
...
2. From the app repo: `agentcore-client connect` (HTTPS URLs + optional CA save)

---
doc_id: ac.doc.sea.client-push-progress-stream
...
summary: Stream the same SyncProgressTracker events from HTTPS ingest-push to
  agentcore-client so operators see percent, ETA, rate, symbols, and phase lines
  matching local agentcore sync.
tags:
- design
- sync
- ingest
...
- backend/packages/agentcore_cli/connect_flow/client_push.py::_run_ingest_push_http
- backend/packages/agentcore_cli/sync_progress/tracker.py::SyncProgressTracker
- backend/packages/agentcore_cli/sync_progress/render.py::print_progress_line
...
`agentcore-client sync` must show the **same live progress UI** as local
`agentcore sync` (percent bar, phase done/total, ETA, rate, queue, symbols/edges,
...
- Manual: `agentcore-client sync` shows percent / ETA / symbols during a large batch.
...
- CLI progress behavior: `docs/08-software-engineering-architecture/42-agentcore-cli-command-reference-continued-continued-continued.md`

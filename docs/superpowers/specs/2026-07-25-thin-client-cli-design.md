---
doc_id: ac.doc.sea.thin-client-cli-design
title: Thin Client CLI (agentcore-client) — Design
...
Separate **client-only** AgentCore installs onto a thin CLI entry (`agentcore-client`) so laptops cannot run server-admin commands, while **server** / **both** keep the full CLI (including client workflows without a second install). Client process control includes scoped remote purge with fail-closed security.
...
flowchart LR
  subgraph clientHost["Client-only host"]
    Thin["agentcore PATH → agentcore-client"]
  end
  subgraph serverHost["Server / both host"]
    Full["agentcore PATH → agentcore_cli full"]
...
| 3 | Client host | `agentcore connect` / `sync` / `purge` / `status` | Remote ops for connect.yaml scope only |
| 4 | Client host | Non-allowlisted command | Absent from parser or denied by role gate |

## Problem

Today `install.sh --role client` skips Compose but still ships the **full** `agentcore` command surface (`graph`, `mcp serve`, `approval`, `service`, `purge` against local stores, etc.). Operators can accidentally (or maliciously) run server-admin operations from a client host. Remote sync/status gaps already showed that “client” was not a real trust boundary.
...
3. Client-only **cannot** perform other AgentCore capabilities (no local stack admin, no cross-scope ops, no server governance).
...
### Client allowlist (`agentcore-client`)

| Command | Purpose |
| --- | --- |
| `connect` | Wire to AgentCore server; persist connect.yaml + MCP |
...
```text
backend/packages/agentcore_cli/          # full CLI (existing) — server / both
backend/packages/agentcore_client/      # thin parser + dispatch — client-only installs
```

`pyproject.toml` scripts:

- `agentcore` → `agentcore_cli.main:main` (server/both PATH target)
- `agentcore-client` → `agentcore_client.main:main` (client-only PATH target; also the implementation behind client’s `agentcore` shim)

Thin package **imports** allowed command handlers from `agentcore_cli` (shared implementation). It must **not** re-export or register server-only subparsers.
...
- Client sync always SSH-remotes to the AgentCore server; interrupt must kill remote pidfile (already designed).
...
4. SSH-run server: `agentcore purge --tenant T --workspace W --project P --yes`.
...
- SSH access to the AgentCore host is a **powerful** credential; this design does not replace SSH hardening.
...
| **T5 — Full CLI left on client PATH** | Client-only install overwrites `~/.local/bin/agentcore` to thin entry; verify in `doctor` / install stage. Invoking full module on client role is gated by allowlist. |
...
1. Thin entry: parsing `agentcore-client service start` (or any non-allowlisted top-level) → argparse error / exit ≠ 0; command not listed in `--help`.
2. `role=client` + `python -m agentcore_cli.main graph ingest …` → denied before side effects.
...
6. Client-only install PATH resolution for `agentcore` resolves to thin main (unit or install-script test).
7. Server/`both` install PATH `agentcore` resolves to full main; `connect` / client sync helpers remain available without installing thin as the primary binary.
8. Allowlist deny in `agentcore_cli.main` does **not** fire when `install_role` is `server` or `both`.

## Implementation sketch

1. Add `backend/packages/agentcore_client/` (`main.py`, `parser.py`, `dispatch.py`, README).
...
5. Gate `agentcore_cli.main._dispatch` for `role=client`.
6. Docs: 36/39/41/42 + client next-steps; this spec remains SoT until product docs absorb it.
7. Tests under `tests/backend/tools/agentcore-cli/` (and client package tests).
...
- [36 - AgentCore CLI](../../08-software-engineering-architecture/36-agentcore-cli.md)

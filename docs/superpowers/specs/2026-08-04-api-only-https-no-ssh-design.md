---
doc_id: ac.spec.api-only-https-no-ssh
title: API-only HTTPS migration without SSH
doc_type: design
status: active
schema_version: '1.0'
owner: platform-engineering
summary: Absolute removal of SSH from the AgentCore product path. All client traffic uses
...
- backend/packages/agentcore_auth/token_registry.py::PostgresAccessTokenRegistry
- backend/packages/agentcore_auth/tokens.py::mint_and_register_access_token
- backend/packages/agentcore_auth/tokens.py::verify_registered_access_token
...
- backend/services/mcp-gateway-service/src/mcp_gateway_service/http_app.py
...
Make AgentCore an **HTTPS-only product** for clients: connect, MCP, sync
...
(re-run `agentcore connect`) to mint a new access token once the current one
expires.
...
- Absolute zero SSH in AgentCore product (CLI client, connect.yaml schema, MCP wiring, docs).
...
| 5 | Client | On 401 / expiry | Re-run `agentcore connect` to re-bootstrap; new token minted |

## TLS auto-certificates

**Layout (SoT):** `<AGENTCORE_DATA_ROOT>/certs/`

| File | Role |
| --- | --- |
| `ca.pem` / `ca.key` | Private CA (client trust) |
| `server.pem` / `server.key` | Leaf for `AGENTCORE_PUBLIC_HOSTNAME` |
| `.agentcore-certs.json` | Marker: hostname, not_after, generated_at |

**Rules**

1. If operator-supplied `AGENTCORE_TLS_CERT` + `AGENTCORE_TLS_KEY` point to existing files → use them; do not auto-gen.
...
| Step | Actor | Action | Outcome |
| --- | --- | --- | --- |
| 1 | Bootstrap | Mint `ac1.*` with unique `jti` | Plaintext returned once in HTTPS response |
...
- Single scoped bearer (`ac1.*` HMAC in `usage_profile.mcp_tokens`, wrapped by
  `agentcore_auth.mint_access_token` / `mint_and_register_access_token`).
...
- No refresh/rotation: once expired, the client re-bootstraps (re-runs
  `agentcore connect`, providing the bootstrap secret again if one is
...
Minted `ac1.*` access tokens carry a unique `jti` claim. `agentcore_auth.token_registry`
...
MCP HTTP token (non-`ac1.*`, lab fallback) carries no `jti` and skips the
registry. Prefer Postgres when
`AGENTCORE_PROJECT_PROFILE_DATABASE_URL` (or the service DSN) is available;
...
2. Client `agentcore connect` posts HTTPS bootstrap with that secret + scope.

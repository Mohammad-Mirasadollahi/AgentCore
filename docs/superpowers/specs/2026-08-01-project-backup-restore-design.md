---
doc_id: ac.doc.ops.project-backup-restore-design
title: Project Backup and Restore — Design
doc_type: feature_spec
status: active
schema_version: '1.0'
owner: platform-engineering
summary: Design for project-scoped .acbak export/import across AgentCore stores,
...
one AgentCore project's analytical state (memories, core data, code graph, docs-sync,
guidance, profiles, embeddings, and related scoped rows) from one server to another
via a single `.acbak` bundle.
...
flowchart TB
  subgraph exportPath [Export]
    CLI1[agentcore_backup_export]
    Orch1[BackupOrchestrator]
    Ports1[StoreExporters]
    CLI1 --> Orch1 --> Ports1 --> Bundle[.acbak]
  end
  subgraph restorePath [Restore]
    Bundle --> CLI2[agentcore_backup_restore]
    CLI2 --> Orch2[BackupOrchestrator]
    Orch2 --> Ports2[StoreImporters]
    Ports2 --> Stores[(PG_and_Neo4j)]
  end
  MCP[agentcore_backup_status_dry_run] --> Orch2
```

| Step | Actor | Action | Outcome |
| --- | --- | --- | --- |
| 1 | Operator | `agentcore backup export` | `.acbak` with manifest, stores, checksums |
| 2 | Operator | Copy bundle to target host | File available offline |
| 3 | Operator | `agentcore backup validate` | Version/checksum gates pass or fail-closed |
| 4 | Operator | `agentcore backup restore` | Scope empty → import; non-empty → fail unless `--replace --yes` |
...
5. **Ownership** — `agentcore_backup` package sequences store exporters/importers (per-schema codecs);
...
presence gate on restore). Archive container is gzip-compressed tar with `.acbak`
...
- Python package `agentcore_backup` is listed in `pyproject.toml` and installs with
  `pip install -e .` / wheelhouse / Docker `COPY backend/packages`.
- `scripts/ensure-venv.sh` and `agentcore doctor` import-check `agentcore_backup`.
...
- Integration round-trip for Postgres-backed stores when `AGENTCORE_DATABASE_URL` is set.
...
- [CLI reference part 4](../../08-software-engineering-architecture/42-agentcore-cli-command-reference-part-4.md)

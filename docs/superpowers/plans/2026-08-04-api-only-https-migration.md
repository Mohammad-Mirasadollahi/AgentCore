---
doc_id: ac.doc.sea.api-only-https-migration-plan
...
- backend/packages/agentcore_cli/connect_config.py
- backend/packages/agentcore_cli/tls_certs.py
---
...
**Goal:** Remove SSH from the AgentCore product path and run all client traffic over HTTPS with auto TLS certificates, a single long-lived scoped access token (SHA-256 digest at rest; never plaintext in DB), and Argon2id hashes for the bootstrap secret at rest.

**Architecture:** One HTTPS edge terminates TLS (auto-CA under `AGENTCORE_DATA_ROOT/certs` when missing). Clients authenticate with a long-lived access Bearer token minted at bootstrap/connect; there is no refresh flow — clients re-bootstrap once the token expires. MCP, connect, and content-push use HTTPS only.

> **Update (2026-08-04):** the refresh-token flow originally shipped in Tasks 2–4/8
> below was removed as YAGNI. `agentcore_auth` now only mints/verifies a single
...
> fail closed with a message to re-run `agentcore connect`. Historical task
> steps below are kept for context but no longer reflect the shipped auth model.
>
> **Update (2026-08-04, hash-at-rest):** minted `ac1.*` tokens carry `jti`;
> `agentcore_auth.token_registry` stores only the SHA-256 digest in
...
**Tech Stack:** FastAPI, httpx, `argon2-cffi`, `cryptography` (X.509), existing `ac1.*` HMAC mint in `usage_profile.mcp_tokens`, Caddy or nginx edge sample under `scripts/install/`.
...
- Auto-certs under `<AGENTCORE_DATA_ROOT>/certs/`; never log private keys.
...
**Files:**
- Create: `backend/packages/agentcore_cli/tls_certs.py`
- Create: `tests/backend/tools/agentcore-cli/test_tls_certs.py`
- Modify: dependency declaration where AgentCore CLI / install declares Python deps (add `cryptography` if missing)
...
def test_ensure_tls_material_creates_once(tmp_path):
    ...
    first = ensure_tls_material(data_root=tmp_path, hostname="agentcore.test")
    assert first.cert_path.is_file()
    assert first.key_path.is_file()
    assert first.ca_pem_path.is_file()
    assert first.generated is True
    second = ensure_tls_material(data_root=tmp_path, hostname="agentcore.test")
...
Run: `.venv/bin/python -m pytest tests/backend/tools/agentcore-cli/test_tls_certs.py::test_ensure_tls_material_creates_once -v`  
Expected: FAIL import / missing module

- [ ] **Step 3: Implement `ensure_tls_material`**

Use `cryptography` to create a private CA + leaf for `hostname` under `{data_root}/certs/`. If `AGENTCORE_TLS_CERT`/`AGENTCORE_TLS_KEY` env paths exist, return those and `generated=False`. chmod keys to `0o600`.
...
**Files:**
- Create: `backend/packages/agentcore_auth/` (minimal package: `hashing.py`, `refresh.py`, `tokens.py`) **or** extend `backend/packages/usage_profile/` if packaging cost is lower — prefer new `agentcore_auth` when it avoids circular imports
- Create: `tests/backend/packages/agentcore_auth/test_refresh_argon2.py`
...
def test_argon2id_roundtrip():
    from agentcore_auth.hashing import hash_secret, verify_secret
...
def test_refresh_rotate_invalidates_old(store):
    from agentcore_auth.refresh import issue_token_pair, rotate_refresh
...
**Files:**
- Modify: [`connect_config.py`](backend/packages/agentcore_cli/connect_config.py) — reject `http://` for `url` / `mcp_http_url` / `graph_url` unless `AGENTCORE_ALLOW_INSECURE_HTTP=1`
- Modify: remove/ignore `server.ssh` load for product path (warn then Phase 3 delete)
- Modify: [`client_push.py`](backend/packages/agentcore_cli/connect_flow/client_push.py) + connect HTTP helpers — on 401, refresh once and retry
- Store refresh via `auth.refresh_token_env` / local secure file under `.agentcore/` mode `0600`
- Test: `tests/backend/tools/agentcore-cli/test_connect_https_and_refresh.py`
...
- [ ] Write example Caddyfile binding `{$AGENTCORE_PUBLIC_HOSTNAME}` with `tls {$CERT} {$KEY}`
...
- Rewrite [`connect_wizard.py`](backend/packages/agentcore_cli/connect_wizard.py) for HTTPS URL + bootstrap secret (no SSH)
- Delete SSH MCP fragment usage from [`run_connect`](backend/packages/agentcore_cli/connect_flow/run.py)
...
- Update `.cursor/rules/agentcore-source-server-path.mdc` (already content-push; ensure no SSH stage)
...
**Files:**
- Create: `backend/packages/agentcore_auth/token_registry.py`
- Modify: `backend/packages/agentcore_auth/tokens.py` (`mint_and_register_access_token`, `verify_registered_access_token`)
...
- Tests: `tests/backend/packages/agentcore_auth/test_token_registry.py`, update `test_auth_refresh.py`

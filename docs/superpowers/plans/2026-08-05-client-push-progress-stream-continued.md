---
doc_id: ac.doc.sea.client-push-progress-stream-plan-continued
...
linked_symbols:
- backend/packages/agentcore_cli/connect_flow/client_push.py::build_push_files
related_docs:
...
`SyncProgressTracker` into `agentcore-client sync`, and doc updates.
...
**Files:**
- Create: `backend/packages/agentcore_cli/connect_flow/push_stream.py`
- Create: `tests/backend/tools/agentcore-cli/test_push_stream.py`
...
PYTHONPATH=backend/packages:backend/services/code-graph-service/src \
  .venv/bin/python -m pytest tests/backend/tools/agentcore-cli/test_push_stream.py -v
```

(Use the same PYTHONPATH pattern as other `tests/backend/tools/agentcore-cli` tests in this repo.)
...
**Files:**
- Modify: `backend/packages/agentcore_cli/connect_flow/client_push.py` (`_run_ingest_push_http`, `client_push_sync`)
- Modify: `tests/backend/tools/agentcore-cli/test_client_push_ingest.py` and/or `test_push_stream.py`

**Interfaces:**
- Consumes: `stream_accept_headers`, `consume_ndjson_ingest_push`, `SyncProgressTracker`
- Produces: live console progress during `agentcore-client sync`
...
.venv/bin/python -m pytest \
  tests/backend/tools/agentcore-cli/test_push_stream.py \
  tests/backend/tools/agentcore-cli/test_client_push_ingest.py -q
```
...
- Modify: Progress bullet in `docs/08-software-engineering-architecture/42-agentcore-cli-command-reference-continued-continued-continued.md` if it currently implies progress is local-only — add that `agentcore-client sync` streams the same tracker lines over HTTPS

- [ ] **Step 1:** Apply factual one-paragraph / table-row updates only (no new speculative features).
- [ ] **Step 2:** `agentcore docs-standards` — new/edited docs conforming.
- [ ] **Step 3:** Manual smoke (optional): on a connected client host, run `agentcore-client sync` and confirm percent / ETA / symbols lines appear during a large batch.

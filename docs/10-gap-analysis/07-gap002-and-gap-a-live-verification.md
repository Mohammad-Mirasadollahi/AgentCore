---
doc_id: ac.doc.gap.gap002-a08-live-verification
...
| Requirement | Detail |
| --- | --- |
| Repo root | `/opt/AgentCore` (or checkout root) |
| Python | `.venv` with `tree-sitter-java` installed |
| Compose data plane | Neo4j + Postgres from AgentCore compose |
| Secrets | Match Compose: `AGENTCORE_NEO4J_PASSWORD`, `AGENTCORE_POSTGRES_PASSWORD` (default lab: `agentcore-local-dev-secret`) |
...
Start application services with the project’s normal host path (`agentcore service start` or equivalent compose app stack). Confirm listeners on the ports above before scenarios.
...
3. Confirm `AGENTCORE_TENANCY_MODE` unset or `shared_scoped`.
...
2. Ingest via live code-graph path (pick one):
   ...
   - `agentcore sync` / `samples/e2e-graph-probe/run_probe.py` pattern against Neo4j stores, or
...
| Guidance resolve | MCP `agentcore_guidance_resolve` or common-context resolve HTTP | bundle `read_model_id=common_context.guidance` |
...
2. Set `AGENTCORE_TENANCY_MODE=db_per_tenant` without `AGENTCORE_TENANT_DATABASE_URL_TEMPLATE` → process/config probe must fail closed.
...
2. Soft mode: unset `AGENTCORE_GUIDANCE_RESOLVE_REQUIRED`; call durable write capability **without** prior guidance resolve → write succeeds with `guidance_hint`.
3. Fail-closed: set `AGENTCORE_GUIDANCE_RESOLVE_REQUIRED=1` on the gateway process; restart or reload env; write without guidance → hard error mentioning guidance resolve.
4. Call `agentcore_guidance_resolve` (or equivalent), then write again → success without fail-closed error.
...
4. With `AGENTCORE_ENFORCE_ADMIN_MATRIX=1` (or actor_roles in payload):
...
Against core-data HTTP (`AGENTCORE_CORE_DATA_PORT`, default `32110`):
...
8. Store an `external_fingerprint` (e.g. `github:pr:123`) and confirm SoR id remains the AgentCore ChangeSet id.

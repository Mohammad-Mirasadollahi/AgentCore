...
- backend/packages/agentcore_cli/commands/ports.py::cmd_ports_show
- backend/packages/agentcore_cli/service_runtime/lifecycle.py::service_state
- tests/backend/tools/agentcore-cli/test_agentcore_cli.py::test_profile_list_and_show
...
- `code_graph_service/domain/hashing.py` (`content_hash`, `HASH_VERSION`, agentcore flags)
...
- `backend/services/rule-engine-service/src/rule_engine_service/bootstrap.py` (`AGENTCORE_RULE_JUDGE=litellm|heuristic`)
...
- `backend/packages/agentcore_sdk/`
...
- `pyproject.toml` packages list (`agentcore_sdk`, `adapter_harness`)
...
- `backend/packages/agentcore_cli/commands/ports.py` (`agentcore ports check`)
- `backend/packages/agentcore_cli/service_runtime/lifecycle.py` (service start preflight)
- `scripts/install/common.sh` (`run_port_preflight`) + stage 04 Compose gate
- `tests/backend/tools/agentcore-cli/test_agentcore_cli.py` (occupied-port + ss owner regression)

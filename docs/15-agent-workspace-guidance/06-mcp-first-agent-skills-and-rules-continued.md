...
1. Call `agentcore_docs_stale_candidates` (default `scope_mode=task_neighborhood`). For discovery use `project_scan` with `path_prefix` and `min_confidence` (destructive act: `0.8`).
...
6. Verify with `agentcore docs-standards` / quality-audit on touched paths.
...
- Ask AgentCore to delete Markdown; AgentCore only surfaces candidates and guidance.
...
- `agentcore_code_graph_unused_candidates` is normative in [`../07-code-knowledge-graph/36-dead-code-candidates-and-cleanup-loop.md`](../07-code-knowledge-graph/36-dead-code-candidates-and-cleanup-loop.md) and is advertised on `programming-cursor-mcp` (scored candidates with `score`/`evidence`/`finding_kind`; default `task_neighborhood` with anchors; optional `project_scan`, `disk_search`, `coverage_hits`, `flag_states`, `triage`).
- `agentcore_docs_stale_candidates` is normative in [`../07-code-knowledge-graph/78-stale-documentation-candidates-and-cleanup-loop.md`](../07-code-knowledge-graph/78-stale-documentation-candidates-and-cleanup-loop.md) (sister loop; prefer update/unlink over delete).

## Acceptance Criteria

- Seed pack defines `mcp-first-agentcore` (including same-change dead-code and stale-docs cleanup, module-contract/README map clauses, and **fix-on-read** for nonconforming product docs + hard-module headers) plus the skills in the matrix (including `agentcore-remove-stale-docs`).
...
- Stale-docs cleanup skill instructs agents to prefer update/unlink, prove before delete, and never asks AgentCore to mutate the repository.

...
AgentCore sits **between** coding agents / IDEs and durable project truth. It is a knowledge + control plane — **not** the executor. External agents and humans still do the work; AgentCore owns ingest, scoped retrieval, governance hooks, and evidence.
...
flowchart TB
  ...
  subgraph Plane["AgentCore plane"]
...
**v1 wedge (what ships first):** connect a repo → `sync` / ingest → code + linked docs graph → MCP explore / hybrid retrieval / change-risk → better agent context. Broader control-plane pieces (tickets, multi-agent routing, approval, cross-domain packs) build on that same plane.

...
Project-scoped `.acbak` export/import for AgentCore.

## Purpose

Own the bundle format, checksum/version gates, and per-store codecs used by
`agentcore backup` and MCP `backup.status` / `backup.dry_run`.
...
Listed in root `pyproject.toml` as `agentcore_backup`. Verified by
`scripts/ensure-venv.sh` and `agentcore doctor` (`import_agentcore_backup`).

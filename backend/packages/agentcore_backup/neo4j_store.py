...
def neo4j_configured(env: dict[str, str] | None = None) -> bool:
    ...
    pwd = str(e.get("AGENTCORE_NEO4J_PASSWORD") or "").strip()
...
def graph_store_expects_neo4j(env: dict[str, str] | None = None) -> bool:
    e = env if env is not None else os.environ
    mode = (
        str(e.get("AGENTCORE_CODE_GRAPH_STORE") or "").strip().lower()
        or str(e.get("AGENTCORE_MCP_GRAPH_MODE") or "").strip().lower()
    )
    return mode == "neo4j"
...
def require_neo4j_for_export(env: dict[str, str] | None = None) -> None:
    ...
    if graph_store_expects_neo4j(env) and not neo4j_configured(env):
        raise RuntimeError(
            "code graph store is neo4j but AGENTCORE_NEO4J_PASSWORD is unset/placeholder; "
...
def _driver():
    ...
    uri = os.environ.get("AGENTCORE_NEO4J_URI", "bolt://127.0.0.1:7687").strip()
    user = os.environ.get("AGENTCORE_NEO4J_USER", "neo4j").strip() or "neo4j"
...
def count_scope_nodes(scope: Scope) -> int:
    if not neo4j_configured():
        return 0
    driver = _driver()
    database = os.environ.get("AGENTCORE_NEO4J_DATABASE", "neo4j").strip() or "neo4j"
    try:

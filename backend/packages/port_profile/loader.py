"""AgentCore development port profile loader, validators, and preflight.

Role: resolve project-scoped non-default ports and block startup on conflicts.
SoT: backend/configs/port-profiles/*.json + AGENTCORE_*_PORT env overrides.
...
DEFAULT_PORT_MAP_REL = Path(".agentcore") / "run" / "port-map.json"

# Listeners we treat as already-ours when re-entering start/install.
_OWN_PROCESS_HINTS = ("docker-proxy", "agentcore", "uvicorn", "mcp-gateway")

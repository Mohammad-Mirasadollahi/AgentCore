...
def ssh_argv(
    ...
    *,
    ...
    OpenSSH joins remote argv with spaces when building the remote command string.
    Passing ``bash -lc 'set -euo …'`` or ``python -c 'import x; …'`` as separate
    list elements therefore loses quoting. Always send one shell-quoted remote string.
    """
    cmd: list[str] = ["ssh"]
    if batch_mode:
    ...
    if identity_file and str(identity_file).strip():
        ...
        cmd.extend(["-i", path, "-o", "IdentitiesOnly=yes"])
    cmd.append(ssh_target)
    if remote_command:
        cmd.append(shlex.join(list(remote_command)))
    return cmd
...
def materialize_ssh_mcp_fragment(
    *,
    ...
    if identity_file and str(identity_file).strip():
        path = str(Path(identity_file).expanduser())
        args.extend(["-i", path, "-o", "IdentitiesOnly=yes"])
    args.append(ssh_target)
    # One remote string so IDE-spawned SSH keeps quoting (same rule as ssh_argv).
    args.append(shlex.join(remote_cmd))
    return {
        "mcpServers": {
            server_name: {
                "command": "ssh",
...
def wire_remote_dev_host(
    *,
    ...
    if not skip_doctor:
        py = remote_python or remote_venv_python(remote_root, remote_os=remote_os)
        if run_ssh(ssh_target, ["test", "-f", py], identity_file=identity_file) != 0:

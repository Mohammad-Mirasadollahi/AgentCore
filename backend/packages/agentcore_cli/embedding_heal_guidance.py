...
def format_embedding_heal_lines(
    ...
    *,
    ...
    if str(sync_mode or "").strip().lower() == "heal":
        lines.append("This run: agentcore sync heal (full-project embedding refresh)")
    else:
        lines.append(
            "Plain agentcore sync only heals embeddings for files touched this run"
        )
        lines.append("Do this: agentcore sync heal")
    return lines
...
def print_embedding_heal_guidance(
    ...
    *,
    ...
    if str(sync_mode or "").strip().lower() == "heal":
        ...
        ui.kv("Do this", "agentcore sync heal")
...
def print_remote_sync_heal_note(*, sync_mode: str = "") -> None:
    ...
    if str(sync_mode or "").strip().lower() == "heal":
        ...
        ui.kv(
            ...
            "re-run with: agentcore sync heal",
        )

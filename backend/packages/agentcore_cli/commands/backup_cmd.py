...
from agentcore_backup.job_state import read_job
...
def _ensure_backup_env() -> None:
    """Load dotenv + compose-derived AGENTCORE_DATABASE_URL (same as status/graph)."""
    load_dotenv_files()
    if str(os.environ.get("AGENTCORE_DATABASE_URL") or "").strip():
        return
    try:

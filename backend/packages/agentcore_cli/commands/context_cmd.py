"""`agentcore context` — measure / stats for native context compression."""
...
def _read_payload(args: argparse.Namespace) -> str:
    ...
    raise SystemExit(
        "Provide --file PATH, --payload TEXT, or pipe stdin into `agentcore context measure`"
    )
...
def cmd_context_stats(args: argparse.Namespace) -> int:
    ...
    report = {
        ...
        "note": "cli_totals accumulate from `agentcore context measure`; MCP uses agentcore_context_stats",
    }
    if getattr(args, "json", False):
        print_json(report)
    else:

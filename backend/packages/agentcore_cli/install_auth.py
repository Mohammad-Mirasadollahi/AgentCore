...
JWT_SECRET_ENV = "AGENTCORE_MCP_TOKEN_SECRET"
...
def upsert_env_key(path: Path, key: str, value: str, *, force: bool = False) -> str:
    ...
    if not found:
        if out and out[-1].strip():
            out.append("")
        out.append(f"# Set by AgentCore install (do not commit)")
        out.append(f"{key}={value}")
        action = "created"
    if action != "preserved":
        path.write_text("\n".join(out) + "\n", encoding="utf-8")
...
def _postgres_url(root: Path) -> str:
    ...
    url = (os.environ.get("AGENTCORE_DATABASE_URL") or "").strip()
    if url:
        return url
    try:
        ...
        return (env.get("AGENTCORE_DATABASE_URL") or "").strip()
    except SystemExit:
        return ""
...
def mint_install_api_key(
    ...
    *,
    ...
    """Mint a scoped ``ac1.*`` API key into the durable token registry (Postgres when available)."""
    from agentcore_auth import PostgresAccessTokenRegistry, mint_and_register_access_token
    from agentcore_auth.token_registry import InMemoryAccessTokenRegistry

...
DEFAULT_CERT_ENV = "AGENTCORE_TLS_CERT"
DEFAULT_KEY_ENV = "AGENTCORE_TLS_KEY"
...
MARKER_NAME = ".agentcore-certs.json"
...
def _ensure_ca_material(certs_dir: Path) -> None:
    ...
    subject = issuer = _build_name(common_name="AgentCore Private CA")
    now = datetime.now(timezone.utc)
    cert = (
...
def _build_name(*, common_name: str) -> x509.Name:
    return x509.Name(
        [
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "AgentCore"),

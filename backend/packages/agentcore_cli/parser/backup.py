...
def register(sub: argparse._SubParsersAction) -> None:
    backup = sub.add_parser(
        "backup",
        help="Export/restore project-scoped .acbak bundles",
    )
    backup_sub = backup.add_subparsers(dest="backup_command", required=True)

    export_p = backup_sub.add_parser("export", help="Export active project scope to .acbak")
    add_scope_args(export_p, required=False)
    export_p.add_argument(
        "--output",
        "-o",
        required=True,
        help="Output .acbak path",
    )

    validate_p = backup_sub.add_parser("validate", help="Validate a .acbak bundle")
    validate_p.add_argument("--input", "-i", required=True, help="Input .acbak path")
    ...
    restore_p = backup_sub.add_parser("restore", help="Restore a .acbak bundle into this server")
    add_scope_args(restore_p, required=False)
    restore_p.add_argument("--input", "-i", required=True, help="Input .acbak path")
    ...
    dry_p.add_argument("--input", "-i", required=True, help="Input .acbak path")

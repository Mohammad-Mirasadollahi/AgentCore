...
- Role: stable fingerprints + cancel-when-cleared + terminal purge for
  sync/quality automated Tasks (ac.doc.core.automated-followup-task-lifecycle-and-retention).
    ...
    *,
    ...
    if include_sync:
        try:
            ...
            try:
                from agentcore_cli.commands.quality_audit.collect import (
...
def retention_days(environ: dict[str, str] | None = None) -> int:
    ...
    raw = str(env.get("AGENTCORE_FOLLOWUP_TASK_RETENTION_DAYS", "") or "").strip()
    if not raw:

...
def resolve_tenancy_mode(environ: dict[str, str] | None = None) -> str:
    ...
    mode = str(env.get("AGENTCORE_TENANCY_MODE") or catalog.get("default_mode") or "shared_scoped").strip()
    modes = {m["mode_id"]: m for m in catalog.get("modes", [])}
    if mode not in modes:
        raise ArchitectureGovernanceError(f"unknown AGENTCORE_TENANCY_MODE={mode!r}")
...
def guidance_resolve_required(environ: dict[str, str] | None = None) -> bool:
    env = environ if environ is not None else os.environ
    raw = str(env.get("AGENTCORE_GUIDANCE_RESOLVE_REQUIRED", "")).strip().lower()

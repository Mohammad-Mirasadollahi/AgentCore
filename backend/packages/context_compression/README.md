...
Native AgentCore context compression (Headroom-inspired, clean-room): shrink bulky
...
| `metrics.py` | Process-local savings counters (`agentcore context stats`) |
...
agentcore context measure --payload '{"rows":[1,2,3]}' --min-chars 1 --json
agentcore context stats
```

`measure` reports one-shot savings and appends to `.agentcore/cache/context-compression-metrics.json`.
`stats` prints those CLI totals. Live MCP gateway counters: `agentcore_context_stats`.

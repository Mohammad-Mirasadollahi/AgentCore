...
def _min_chars() -> int:
    raw = os.environ.get("AGENTCORE_CONTEXT_COMPRESS_MIN_CHARS", "2000").strip()
    try:
...
def _max_string() -> int:
    raw = os.environ.get("AGENTCORE_CONTEXT_COMPRESS_MAX_STRING", "240").strip()
    try:
...
def _max_list() -> int:
    raw = os.environ.get("AGENTCORE_CONTEXT_COMPRESS_MAX_LIST", "24").strip()
    try:

...
Forbidden: raising into service boot for missing optional wheel.
"""
...
def try_build_accelerator(
    *,
    ...
    if adapter is None:
        return None, None
    table = (id_map_table or str(env.get("AGENTCORE_TURBOVEC_ID_MAP_TABLE", "")).strip() or "")
    url = (database_url or str(env.get("AGENTCORE_TURBOVEC_ID_MAP_DATABASE_URL", "")).strip() or "")
    if url and table:
        try:
            return adapter, PostgresEntityIdMap(url, table=table)
        except Exception:
            return adapter, InMemoryEntityIdMap()
    return adapter, InMemoryEntityIdMap()

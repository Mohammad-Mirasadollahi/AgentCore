"""ANN accelerator env config (AGENTCORE_RAG_ANN_* / AGENTCORE_TURBOVEC_*)."""
...
class AnnAcceleratorConfig:
    ...
    def from_environment(cls, environ: dict[str, str] | None = None) -> AnnAcceleratorConfig:
        ...
        raw_acc = str(env.get("AGENTCORE_RAG_ANN_ACCELERATOR", "off")).strip().lower() or "off"
        if raw_acc not in _ALLOWED_ACCELERATORS:
            raise ValueError(
                "AGENTCORE_RAG_ANN_ACCELERATOR must be 'off' or 'turbovec', "
                f"got {raw_acc!r}"
            )
        raw_bw = str(env.get("AGENTCORE_TURBOVEC_BIT_WIDTH", "4")).strip() or "4"
        try:
            bit_width = int(raw_bw)
        except ValueError as exc:
            raise ValueError("AGENTCORE_TURBOVEC_BIT_WIDTH must be 2, 3, or 4") from exc
        if bit_width not in _ALLOWED_BIT_WIDTHS:
            raise ValueError("AGENTCORE_TURBOVEC_BIT_WIDTH must be 2, 3, or 4")
        snapshot_uri = str(env.get("AGENTCORE_TURBOVEC_SNAPSHOT_URI", "")).strip()
        sync_mode = str(env.get("AGENTCORE_TURBOVEC_SYNC_MODE", "sync_on_write")).strip().lower() or "sync_on_write"
        if sync_mode not in _ALLOWED_SYNC_MODES:
            raise ValueError(
                "AGENTCORE_TURBOVEC_SYNC_MODE must be 'sync_on_write' or 'async_job', "
                f"got {sync_mode!r}"
            )
        return cls(
            accelerator=raw_acc,

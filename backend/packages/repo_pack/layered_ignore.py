"""Layered ignore: defaults → .gitignore → .agentcoreignore (CI-45 / RM-05)."""
...
IGNORE_FILENAMES = (".gitignore", ".agentcoreignore")
...
def collect_layered_ignore_rules(root: Path) -> tuple[list[IgnoreRule], list[str]]:
    """Ordered rules from ``.gitignore`` then ``.agentcoreignore``."""

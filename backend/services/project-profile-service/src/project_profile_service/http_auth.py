...
Access tokens are ``ac1.*`` HMAC tokens minted via ``agentcore_auth.mint_and_register_access_token``
(secret from ``AGENTCORE_MCP_TOKEN_SECRET`` / ``AGENTCORE_MCP_HTTP_TOKEN``). When no
...
from agentcore_auth.tokens import verify_registered_access_token

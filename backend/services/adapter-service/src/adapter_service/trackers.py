...
def _load_vendor_class(subdir: str, class_name: str) -> Any:
    ...
    spec = spec_from_file_location(f"agentcore_tickets_{subdir.replace('-', '_')}", path)
...
def build_tracker_registry(environ: dict[str, str] | None = None) -> dict[str, Any]:
    ...
    jira_url = str(env.get("AGENTCORE_JIRA_BASE_URL") or "").strip()
    jira_email = str(env.get("AGENTCORE_JIRA_EMAIL") or "").strip()
    jira_token = str(env.get("AGENTCORE_JIRA_API_TOKEN") or "").strip()
    jira_project = str(env.get("AGENTCORE_JIRA_PROJECT_KEY") or "").strip()
    ...
    linear_key = str(env.get("AGENTCORE_LINEAR_API_KEY") or "").strip()
    linear_team = str(env.get("AGENTCORE_LINEAR_TEAM_ID") or "").strip()
    ...
    gh_token = str(env.get("AGENTCORE_GITHUB_TOKEN") or "").strip()
    gh_owner = str(env.get("AGENTCORE_GITHUB_OWNER") or "").strip()
    gh_repo = str(env.get("AGENTCORE_GITHUB_REPO") or "").strip()

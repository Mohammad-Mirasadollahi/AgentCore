...
class Settings:
    ...
    def from_environment(cls) -> "Settings":
        database_url = os.environ.get("AGENTCORE_DOCS_SYNC_DATABASE_URL", "").strip()
        if not database_url:
            raise RuntimeError("AGENTCORE_DOCS_SYNC_DATABASE_URL is required")
        if not database_url.startswith(("postgresql://", "postgresql+psycopg://")):
            raise RuntimeError("AGENTCORE_DOCS_SYNC_DATABASE_URL must use PostgreSQL")
...
def build_container(settings: Settings | None = None) -> ServiceContainer:
    ...
    return ServiceContainer(service=DocsSyncService(PostgresStore(resolved.database_url)), settings=resolved)

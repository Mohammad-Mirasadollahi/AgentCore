...
2. Edit models, keys, store settings, and (optionally) `AGENTCORE_RAG_ANN_ACCELERATOR` in that root `.env`.
...
5. ANN default is `off` (pgvector Stage-1 only). To enable Turbovec Stage-2, install `.[turbovec]`, set `AGENTCORE_RAG_ANN_ACCELERATOR=turbovec`, then run `python -m vector_index.promotion_gate`.

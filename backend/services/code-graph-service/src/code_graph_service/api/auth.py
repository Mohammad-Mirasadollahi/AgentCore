...
_TOKEN_ENVS = (
    "AGENTCORE_CODE_GRAPH_HTTP_TOKEN",
    "AGENTCORE_CONNECT_TOKEN",
    "AGENTCORE_GRAPH_HTTP_TOKEN",
)
...
def require_content_push_http_auth(
    ...
    - ``ac1.*`` access token (agentcore_auth / connect bootstrap) → verified via
    ...
    if not expected:
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=(
                "content-push HTTP requires AGENTCORE_CODE_GRAPH_HTTP_TOKEN "
                "(or AGENTCORE_CONNECT_TOKEN) when not on loopback"
            ),
            headers={"WWW-Authenticate": "Bearer"},
        )

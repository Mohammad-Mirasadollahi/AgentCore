...
SoT / invariants: HASH_VERSION bumps invalidate prior digests; agentcore doc-flags must affect hash.
Allowed failures: SyntaxError/parse failure falls back to normalized source digest.
Forbidden failures: stripping string-literal comment markers; ignoring agentcore: flags.
"""
...
def _canonical_flag(raw: str) -> str:
    match = _AGENTCORE_BODY.search(raw.strip())
    if not match:
        return raw.strip()
    return f"# agentcore:{match.group(1).strip()}"


def extract_agentcore_flags(source: str, language: str = "python") -> list[str]:
    """Collect AgentCore doc-flag comments (string-literal safe for Python)."""
    lang = (language or "python").strip().lower() or "python"
    if lang == "python":
        return _extract_python_flags(source)
    flags: list[str] = []
    for line in source.splitlines():
        if _AGENTCORE_FLAG.search(line):
...
def _extract_python_flags(source: str) -> list[str]:
    ...
    try:
        for tok in tokenize.generate_tokens(io.StringIO(source).readline):
            if tok.type == tokenize.COMMENT and _AGENTCORE_FLAG.search(tok.string):
                flags.append(_canonical_flag(tok.string))
    except (tokenize.TokenError, IndentationError):
        for line in source.splitlines():
            stripped = line.strip()
            if stripped.startswith("#") and _AGENTCORE_FLAG.search(stripped):
...
def normalize_source(source: str, language: str = "python") -> str:
    ...
    flags = extract_agentcore_flags(source, normalized_language)
...
def _python_ast_material(source: str) -> str:
    flags = extract_agentcore_flags(source, "python")
    try:
...
def _normalize_c_family_fallback(source: str) -> str:
    ...
    while i < n:
        ...
        if ch == "/" and nxt == "/":
            ...
            if _AGENTCORE_FLAG.search(comment):
        ...
        if ch == "/" and nxt == "*":
            ...
            if _AGENTCORE_FLAG.search(block):

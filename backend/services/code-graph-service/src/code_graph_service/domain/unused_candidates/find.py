...
scores only decrease; AgentCore never mutates the repository.
...
from ..dead_code_scoring import flag_controlled_dead_port
...
def find_unused_candidates(
    ...
    *,
    ...
    for sid in sorted(dead_ids):
        ...
        test_only = test_only_for_symbol(sid, by_id, strong_sources, edges)
        if test_only:
            ...
            finding_kind = "unused_symbol"
        file_live = file_has_live_importers(
            symbol.file_path, symbols, edges, live_ids | (all_ids - pool_ids)
        )
        row = build_row(
            symbol,
    ...
    for row in zombie_package_candidates(
        symbols, edges, pool_ids, dead_ids, freshness=freshness
    ):
        append_row(
            row,
            min_confidence=min_confidence,

...
def build_row(
    ...
    *,
    ...
    dynamic = collect_directory_dynamic_taint(all_symbols, path)
    path_risk = path_has_runtime_load_risk(path)
    wip_path = path_looks_wip(path)

    def _name_search(name: str, p: str) -> list[str]:
        hits = list(graph_corpus_string_name_hits(all_symbols, name, p))
    ...
    scored = score_candidate(
        ScoreInput(
            ...
            days_since_touch=days_since_touch_from_symbol(symbol),
        )
    )

...
Forbidden: raising confidence from weak signals; AgentCore never deletes code.
"""
...
def days_since_touch_from_symbol(symbol: Any) -> int | None:
    """Optional freshness ranking from symbol metadata / updated_at."""
    ...
    if raw is None:
        raw = getattr(symbol, "updated_at", None) or ""
    if not raw:
        return None
    try:

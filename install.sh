...
#
# Docs: docs/08-software-engineering-architecture/39-local-install-runbook.md
set -euo pipefail
...
Options:
  ...
  --skip-prerequisites    Do not apt-install; fail if deps missing
  --skip-infra            Skip Compose env + PostgreSQL/Neo4j containers
...
Default flow (all stages):
  01_prerequisites → 02_venv → 03_compose_env → 04_docker_infra → 05_verify
